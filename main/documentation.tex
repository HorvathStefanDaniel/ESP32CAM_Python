...
GET \texttt{/mode?m=on|off|rainbow} & Set whole strip to one mode \\
GET \texttt{/split?left=L\&right=R} & Left half mode \texttt{L}, right half mode \texttt{R} (\texttt{L},\texttt{R} = \texttt{on}, \texttt{off}, or \texttt{rainbow}) \\
...
OPEN\_HAND   & ON (whole strip or left/right half in split) \\
OK           & OFF (whole strip or half) \\
HANG\_LOOSE  & Rainbow mode (thumb + pinky; groovy/chill; whole strip or half) \\
THUMB\_UP    & BRIGHT+ \\
THUMB\_DOWN  & BRIGHT- \\
...
\end{tabular}
\end{center}
\textbf{One hand:} the whole strip follows that gesture (on, off, or rainbow). \textbf{Two hands} (both showing a mode gesture): left half follows hand 0, right half follows hand 1 (e.g.\ left OK + right open palm = left half off, right half on).
