...
The hand gesture script controls a separate ESP32-driven addressable LED strip over HTTP: OPEN\_HAND turns lights on, OK turns them off, THUMB\_UP/DOWN changes brightness, POINTER controls moving fun clusters, PEACE triggers position-driven ripples, PINCH triggers explosion effects, and HANG\_LOOSE/ROCK/THREE select rainbow, strobe, and breathe modes.

Optional ways to start the different scripts include \texttt{gesture\_launcher.py} (a small UI to paste camera and LED strip IPs and start the hand gesture script), \texttt{run.ps1} (PowerShell launcher that creates a virtual environment, installs dependencies, and runs a chosen script), and \texttt{run.exe} (a double-clickable Windows executable built from \texttt{run\_launcher.py} that offers the same behaviour with an interactive menu for script choice, webcam, URLs, and options).
...
\begin{itemize}
    ...
    \item \textbf{Hand gestures:} MediaPipe Hand Landmarker; OPEN\_HAND/OK (on/off), THUMB\_UP/DOWN (brightness), POINTER (fun), PEACE (ripple), PINCH (explosion), HANG\_LOOSE/ROCK/THREE (modes)
    \item \textbf{LED strip control:} HTTP API (on/off, fun position/hue, brightness); serial commands for the LED board
    \item \textbf{Launcher:} \texttt{run.ps1} for venv and script selection; \texttt{run.exe} (built from \texttt{run\_launcher.py}) for a double-click menu with webcam/URL/recognition prompts; \texttt{gesture\_launcher.py} for camera/LED IPs and Start
\end{itemize}
...
\subsection{Creating and using \texttt{run.exe}}
A Windows executable \texttt{run.exe} can be built so that scripts can be started by double-clicking, without opening a terminal or running PowerShell. It behaves like \texttt{run.ps1} but adds an interactive menu when started with no arguments.

\textbf{How to create \texttt{run.exe}:}
\begin{enumerate}
    \item Open a terminal in the project directory (the \texttt{main} folder containing \texttt{run.ps1} and the \texttt{.py} scripts).
    \item Run: \texttt{python build\_run\_exe.py}. The script installs PyInstaller if needed, then builds a single \texttt{run.exe} in the same folder.
    \item Keep \texttt{run.exe} in that project folder; it uses the folder to find \texttt{.venv}, \texttt{requirements.txt}, and the Python scripts.
\end{enumerate}

\textbf{How to use \texttt{run.exe}:}
\begin{itemize}
    \item \textbf{Double-click} (or run with no arguments): A console menu appears. Choose the script (1 = object\_count, 2 = face\_detect, 3 = hand\_gesture, 4 = gesture\_launcher). Then you are prompted for: use webcam (y/n), camera/stream URL if not webcam, and for \texttt{hand\_gesture} the optional LED strip URL. For \texttt{face\_detect} you are also asked whether to use face recognition. After that, the chosen script runs with those options.
    \item \textbf{Command line:} \texttt{run.exe} \textit{script} [\textit{args...}] runs the script directly, e.g.\ \texttt{run.exe hand\_gesture --webcam --led-url http://192.168.1.101}. No menu is shown; arguments are passed through as with \texttt{run.ps1}.
\end{itemize}

The executable is built from \texttt{run\_launcher.py}, which performs the same venv creation, dependency install, and \texttt{PYTHONPATH} setup as \texttt{run.ps1}. \texttt{run.exe} is listed in \texttt{.gitignore}; rebuild after updating \texttt{run\_launcher.py} if you change the launcher behaviour.

\subsection{Portable package (no Python on target PC)}
To run the project on a Windows PC that does not have Python, CMake, or C++ build tools installed, build a \textbf{portable package} once on a machine that has Python 3.9+ and pip:

\begin{enumerate}
    \item On a PC with Python installed, open a terminal in the project directory (\texttt{main}).
    \item Run: \texttt{python build\_portable.py}. The script installs PyInstaller and \texttt{requirements.txt}, then builds standalone \texttt{.exe} files for each script and the launcher. This can take several minutes (especially \texttt{hand\_gesture.exe} with MediaPipe).
    \item The output folder \texttt{dist\_portable} will contain \texttt{run.exe}, \texttt{hand\_gesture.exe}, \texttt{face\_detect.exe}, \texttt{object\_count.exe}, and \texttt{gesture\_launcher.exe}.
    \item Copy the entire \texttt{dist\_portable} folder to the target PC (e.g.\ USB stick or network). No Python or other runtimes need to be installed on that PC.
\end{enumerate}

\textbf{On the target PC:} Double-click \texttt{run.exe} for the interactive menu, or run any script directly (e.g.\ \texttt{hand\_gesture.exe --webcam --led-url http://LED\_IP}). For face recognition, create a \texttt{known\_faces} folder next to the executables and add reference photos. The folder includes \texttt{README\_portable.txt} with short usage notes. Building the portable package is one-time (or after code changes); end users only need the copied folder.

\subsection{Libraries to install}
See \textbf{Environment and Dependencies} for what to install (Python version, CMake if needed, and step-by-step setup). Summary of \textbf{Python} packages (install via \texttt{pip install -r requirements.txt} in the project \texttt{.venv}):
\begin{itemize}
    \item \texttt{opencv-python} ($\geq$4.8.0) -- used by all vision scripts (stream\_reader, object\_count, face\_detect, hand\_gesture)
    \item \texttt{requests} ($\geq$2.28.0) -- HTTP for stream and LED strip
    \item \texttt{mediapipe} ($\geq$0.10.0) -- hand landmarks for \texttt{hand\_gesture.py} (requires Python 3.9+)
    \item \texttt{face\_recognition} ($\geq$1.3.0) -- optional; uncomment in \texttt{requirements.txt} to enable \texttt{face\_detect.py --recognize}. Depends on \texttt{dlib}; on some systems pip may need CMake/C++ tools to build it.
\end{itemize}
...
\subsection{What you need to install (PC / development)}
To run the Python scripts on your PC, install the following. If you only want to run the \textbf{portable package} (\texttt{dist\_portable}) on another machine, you do \emph{not} need to install anything on that machine; see the portable package section.

\begin{itemize}
    \item \textbf{Python 3.9, 3.10, or 3.11} (64-bit recommended). Required for all scripts; \texttt{hand\_gesture.py} and \texttt{gesture\_launcher.py} need at least 3.9 (MediaPipe 0.10+).
    \begin{itemize}
        \item \textbf{Windows:} Install from \href{https://www.python.org/downloads/}{python.org} or run \texttt{python} in a terminal to open the Microsoft Store. During setup, enable ``Add Python to PATH''.
        \item \textbf{Linux/macOS:} Use the system package manager or \href{https://www.python.org/downloads/}{python.org}. Ensure \texttt{python3} and \texttt{pip} are available.
    \end{itemize}
    \item \textbf{pip} (Python package installer). Usually included with Python. Check with \texttt{pip --version}.
    \item \textbf{Python libraries} from \texttt{requirements.txt}: \texttt{opencv-python}, \texttt{requests}, \texttt{mediapipe}. Optional: \texttt{face\_recognition} for face recognition in \texttt{face\_detect.py} (uncomment in \texttt{requirements.txt}). Install with \texttt{pip install -r requirements.txt} inside the project virtual environment (see below).
    \item \textbf{CMake and C++ build tools} (only if you use \texttt{face\_recognition} and pip cannot install a prebuilt \texttt{dlib} wheel). On Windows, if \texttt{pip install face\_recognition} fails to find a wheel, install \emph{one} of:
    \begin{itemize}
        \item \textbf{Visual Studio Build Tools} (\href{https://visualstudio.microsoft.com/visual-cpp-build-tools/}{Build Tools for Visual Studio}) with the ``Desktop development with C++'' workload, or
        \item \textbf{CMake} (\href{https://cmake.org/download/}{cmake.org}) and a C++ compiler (e.g. MinGW or Visual Studio).
    \end{itemize}
    Many users on Windows get a prebuilt \texttt{dlib} wheel and do not need CMake. If \texttt{pip install -r requirements.txt} completes without errors, you can skip this.
\end{itemize}

\subsection{Python setup (venv and dependencies)}
From the project directory (\texttt{main}):
...
\end{lstlisting}
This creates a virtual environment (\texttt{.venv}) and installs \texttt{opencv-python}, \texttt{requests}, \texttt{mediapipe}. To enable face recognition, uncomment \texttt{face\_recognition} in \texttt{requirements.txt} and run \texttt{pip install -r requirements.txt} again.
...
Figure \ref{fig:system} shows the high-level flow: ESP32-CAM streams MJPEG to the PC; Python scripts use \texttt{stream\_reader} to get frames; \texttt{hand\_gesture} sends HTTP requests to the ESP32 LED strip. The launchers (\texttt{run.ps1}, \texttt{run.exe}, \texttt{gesture\_launcher}) are optional entry points.
...
\begin{tikzpicture}[node distance=1.2cm,>=Stealth,every node/.style={font=\small}]
  ...
  \node[draw,rectangle,rounded corners,below=0.6cm of reader] (run) {run.ps1 / run.exe / gesture\_launcher};
...
\subsection{run.ps1 / run.exe flow}
Figure \ref{fig:runps1} outlines the launcher: script name and arguments (or the interactive menu when using \texttt{run.exe} with no args) trigger venv creation (if missing), Python 3.9+ check for hand\_gesture/gesture\_launcher, dependency install, \texttt{PYTHONPATH} set, and script execution.
...
\begin{tikzpicture}[node distance=1cm,>=Stealth,every node/.style={font=\small,draw,rectangle}]
  ...
  \draw[->] (path) -- (run);
\end{tikzpicture}
\caption{run.ps1 / run.exe flow from script selection to execution.}
\label{fig:runps1}
\end{figure}
...
\textbf{Step 2 -- Finger states:} For each finger, the code decides ``extended'' or ``folded'' using simple geometry. The \textbf{thumb} is ``down'' (folded) when the \textbf{thumb tip only} is past the line from wrist to index root (INDEX\_MCP); only the tip is used (not the IP joint), for a more comfortable pose. Otherwise the thumb is extended. For \textbf{index, middle, ring, pinky}, a finger is ``extended'' if the tip is above the PIP joint in image coordinates \textbf{or} the tip is farther from the wrist than the PIP (so pointing sideways still counts as extended).

\textbf{Step 3 -- Gesture classification:} The mapping from finger states to gesture names is \textbf{rule-based}, not a neural network. Examples: \textbf{Pinch} = thumb--index tip distance $<$ about 30\% of ``hand size'' (e.g.\ wrist to middle base), other fingers folded. \textbf{OK} = same pinch distance but middle, ring, pinky extended. \textbf{Fist} = no fingers extended. \textbf{Open hand} = four or more extended. \textbf{Thumb up/down} = only thumb extended; tip vs joint in $y$ distinguishes them. \textbf{Pointer} = index extended, others folded. \textbf{PEACE} = index + middle extended, ring and pinky folded, thumb folded. \textbf{THREE} = index + middle + thumb extended, ring and pinky folded. \textbf{ROCK} = index + pinky extended, middle and ring folded (thumb optional). These definitions are tuned for comfort.

\textbf{Step 4 -- Stability:} \textbf{Debouncing:} The script uses two debounce windows. A short window (\texttt{DEBOUNCE\_FRAMES = 3}) stabilizes the on-screen gesture label, and a stricter window (\texttt{LED\_DEBOUNCE\_FRAMES = 5}) is required before any LED command is sent. This means the LED strip only changes after the same gesture appears in 5 consecutive frames, which reduces flicker from brief misdetections.

\textbf{Python implementation (LED debounce):}
\begin{lstlisting}[language=Python]
LED_DEBOUNCE_FRAMES = 5

# Per-hand rolling history
...
# Stable only if all values in the debounce window match
g0_stable = None
...
Run via \texttt{run.ps1}, \texttt{run.exe}, or directly with the project directory on \texttt{PYTHONPATH}. [Text in square brackets are comments and should not be included when using the commands in the terminal.]

\textbf{PowerShell (\texttt{run.ps1}):}
\begin{lstlisting}[language=bash]
...
.\run.ps1 hand_gesture --url ... --led-url http://LED_IP --webcam --mirror ...
.\run.ps1 gesture_launcher
\end{lstlisting}

\textbf{Executable (\texttt{run.exe}):} Double-click \texttt{run.exe} for an interactive menu (choose script, then answer prompts for webcam, URL, face recognition, LED URL). Or from a terminal: \texttt{run.exe hand\_gesture --webcam --led-url http://LED\_IP}, \texttt{run.exe face\_detect --recognize}, etc.
...
GET \texttt{/rainbow} or \texttt{/groovy} & Whole strip rainbow effect (persistent mode) \\
GET \texttt{/mode?m=on|off|rainbow|strobe|breathe} & Set whole strip to one mode \\
GET \texttt{/split?left=L\&right=R} & Left/right half modes (\texttt{L},\texttt{R} = \texttt{on}, \texttt{off}, \texttt{rainbow}, \texttt{strobe}, \texttt{breathe}) \\
GET \texttt{/ripple?h=H} & One-shot color wave across strip (hue \texttt{H} 0--360) \\
...
Serial (115200): \texttt{on}, \texttt{off}, \texttt{status} / \texttt{?}, \texttt{bright+}, \texttt{bright-}.
...
OPEN\_HAND   & ON (whole strip or half in split) \\
OK           & OFF (whole strip or half) \\
HANG\_LOOSE  & Rainbow mode (thumb + pinky; whole strip or half) \\
PEACE        & Ripple effect (index + middle, thumb folded; position-driven, dark strip) \\
ROCK         & Strobe/party mode (index + pinky; whole strip or half) \\
THREE        & Breathe/pulse mode (index + middle + thumb; whole strip or half) \\
THUMB\_UP    & BRIGHT+ \\
THUMB\_DOWN  & BRIGHT- \\
POINTER      & Fun mode: 1 or 2 moving clusters (index position and hue) \\
PINCH        & Explosion from hand position (1 or 2 hands) \\
\bottomrule
\end{tabular}
\end{center}
Thumb is ``down'' when only the thumb \emph{tip} is past the wrist--index-root line (comfort tuning). \textbf{One hand:} the whole strip follows that gesture. \textbf{Two hands} (both showing a mode gesture): left half = hand 0, right half = hand 1. Modes: on, off, rainbow, strobe, breathe.
...
\begin{itemize}
    ...
    \item \texttt{run\_launcher.py} -- Python launcher (same logic as run.ps1; interactive menu for script, webcam, URL, recognition)
    \item \texttt{build\_run\_exe.py} -- Builds \texttt{run.exe} from \texttt{run\_launcher.py} using PyInstaller (run once; \texttt{run.exe} appears in project folder)
    \item \texttt{build\_portable.py} -- Builds a full portable package (\texttt{dist\_portable/}) with standalone \texttt{.exe} files so no Python is needed on the target PC
    \item \texttt{run.exe} -- Windows executable (build with \texttt{python build\_run\_exe.py} or \texttt{build\_portable.py}); double-click for menu or pass script name and args; in \texttt{dist\_portable} it runs the sibling script \texttt{.exe} files without Python
